...
| Copy a recent dictation | Turn on **Clipboard history** (setup or Settings › **Clipboard**), then menu bar › **Clipboard** › click one. Kept in memory only; **Clear** forgets them. |
...
keep_history = false           # true: menu bar › Clipboard lists recent dictations (memory only)
...
- **Transcripts** are never written to disk or logs. They're printed only when you run with `-v`. The optional **Clipboard history** (off by default) keeps the last few in memory only, until sayit quits.
...
The app also contains open-source libraries under their own licenses, all permissive (mostly MIT and Apache-2.0; one, `option-ext`, is MPL-2.0), including Microsoft's ONNX Runtime (MIT). Their notices ship inside the app (Settings › About › **Licenses**, or `sayit.app/Contents/Resources/THIRD-PARTY-NOTICES.txt`), generated by `scripts/third-party-notices.py`.
