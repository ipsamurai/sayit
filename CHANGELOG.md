...
- Recent Dictations (off by default; turn it on in setup or Settings › Clipboard): the last 3, 5 or 10 dictations, kept in memory only, listed in the menu bar. Click one to copy it; Clear Recent forgets them.
- Settings › General: change the hotkey and Hold/Toggle mode while sayit runs, with no restart.
- Settings › General: **Start sayit at login** (through macOS's login items; the installed app only), and a choice of what closing Settings does: keep running in the menu bar, keep running with a Dock icon too, or quit (`on_close` in config.toml).
